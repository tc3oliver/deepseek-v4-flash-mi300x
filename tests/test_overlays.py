...
Provenance diff freshness (network-based) is NOT tested here — it runs exactly
once in CI as a standalone step (``python3 scripts/validate_diff_freshness.py``
in ``.github/workflows/validate.yml``) so upstream files are fetched only once
per run. See ``tests/test_diff_freshness.py`` for the offline logic tests.
"""
